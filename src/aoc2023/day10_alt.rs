...
fn part2() -> AocRes {
    ...
    let mut system = _get_data("10.txt.test2c");
    system.calc_distance();
    let graph = system.graph;
    let wall_counter = do_the_accumulate(&graph);

    // wall_counter.values().for_each(|n2| {
    //     if n2.is_within() {
    //         tprint!(n2);
    //     };
    // });
...
fn _accumulate_walls(start: Point, dir: Dir, graph: &Graph, wall_counter: &mut WallCounter) {
    // need to keep a running tally of walls seen from which direction
    // so it should be something like HashMap<Dir, HashMap<Dir, u32>>
    // TODO
    let mut cur = start;
    ...
    while let Some(n) = graph.get(&cur) {
        if n.distance.is_some() && (n.connections.contains(&d1) || n.connections.contains(&d2)) {
            assert!(n.type_ != '.');
            walls_seen += 1;
        }
        if n.type_ == '.' {
...
impl Node2 {
    fn is_within(&self) -> bool {
        self.counts.values().sum::<u32>().is_odd()
    }
}
